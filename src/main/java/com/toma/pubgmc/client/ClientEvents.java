	...
	public void renderOverlayPost(RenderGameOverlayEvent.Post e)
	{
    	...
    	if(!e.isCancelable() && e.getType() == ElementType.TEXT && sp.getHeldItemMainhand().getItem() instanceof GunBase)
    	{
    		...
    		if(weaponStack.hasTagCompound())
    		{
        		...
        		mc.fontRenderer.drawStringWithShadow(gun.getItemStackDisplayName(new ItemStack(gun)) + ": " + gun.getFiremode(), x + ConfigPMC.vr.wepStatusX, y - 9 + ConfigPMC.vr.wepStatusY, 16777215);
    			mc.fontRenderer.drawStringWithShadow(TextFormatting.BOLD + "" + ammo + TextFormatting.RESET + " | " + totalCount, x + ConfigPMC.vr.wepStatusX, y + ConfigPMC.vr.wepStatusY, 16777215);
    		}
    	}
