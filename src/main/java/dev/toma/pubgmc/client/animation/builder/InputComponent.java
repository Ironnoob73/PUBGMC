...
public class InputComponent extends UIComponent {
    ...
    public void draw(FontRenderer renderer) {
        ...
        if(selected) {
            RenderHelper.drawColoredShape(x, y, x + w, y + h, 0.0F, 0.7F, 0.0F, 1.0F);
        } else {
            RenderHelper.drawColoredShape(x, y, x + w, y + h, 1.0F, 1.0F, 1.0F, 1.0F);
        }
        RenderHelper.drawColoredShape(x + 1, y + 1, x + w - 1, y + h - 1, 0.0F, 0.0F, 0.0F, 1.0F);
        String s = name.get();
